    ...
    def __init__(self):
        ...
        self.intent_patterns = {
            'system_control': [
                ...
                r'(bluetooth|wifi|wireless)',
                r'(enable|disable|turn\s+(on|off))\s+(bluetooth|wifi)',
                r'(brightness|screen)\s+(up|down|increase|decrease)',
                r'(volume|sound)\s+(up|down|increase|decrease)'
            ],
            'app_control': [
        ...
        self.entity_patterns = {
            ...
            'system_feature': r'(bluetooth|wifi|wireless|brightness|volume|sound)',
            'action': r'(enable|disable|turn\s+(on|off)|increase|decrease|up|down)'
        }
    ...
    def extract_parameters(self, text: str, intent: str) -> Dict[str, str]:
        ...
        if intent == 'system_control':
            if 'volume' in text_lower:
                ...
                if percent_match:
                    parameters['value'] = percent_match.group(1)
                ...
                parameters['control_type'] = 'bluetooth'
                if any(word in text_lower for word in ['disable', 'turn off', 'off']):
                    parameters['action'] = 'disable'
                elif any(word in text_lower for word in ['enable', 'turn on', 'on']):
